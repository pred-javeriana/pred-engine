...
def _es_marco_interno_guardia(archivo: Path, funcion: str) -> bool:
    """Omite marcos del propio modulo y de pathlib/io usados por Path.open()."""
    if archivo == Path(__file__).resolve():
        return True
    if funcion == "guarded_open":
        return True
    ruta = str(archivo).lower()
    return funcion == "open" and "pathlib" in ruta


def _nombre_llamador() -> str:
    for frame_info in inspect.stack()[1:]:
        archivo = Path(frame_info.filename).resolve()
        if _es_marco_interno_guardia(archivo, frame_info.function):
            continue
...
def raw_read_only_guard(raw_root: str | Path) -> Iterator[None]:
    ...
    # Path.open() usa io.open, no builtins.open; hay que parchear ambos.
